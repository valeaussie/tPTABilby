...
#CHAIN_FILE = "/fred/oz005/users/vdimarco/tBilby/results/chains/singlePsrNoise/J1713+0747_tm+ef+ec+eq+rn+dm+chrom+gw_1/chain_1.txt"
...
FIG = os.path.join(DIR_SAVE, f"ppc_tPTABilby_Enterprise_{PULSAR}.pdf")
...
print("Matt (txt) params non-None:", {k: v is not None for k, v in params_Matt.items()})
...
plt.fill_between(f, lo_pref, hi_pref, alpha=0.25, label="Enterprise 68%")
plt.loglog(f, med_pref, color="C1", lw=1.0, label="Enterprise median")

# Final plot settings
plt.figure(1)
# ----- Final PPC plot settings and save -----
plt.xlabel("Frequency [Hz]", fontsize=14)
#plt.ylabel("h_c(f)" if USE_CHAR_STRAIN else "P(f) [s$^2$/Hz]", fontsize=14)
...
# # =========================
# # 6. Plot CH distributions (nested TB, posterior TB, Matt)
# # =========================
...
# # tBilby posterior samples
# if CH_A in post_tb.columns:
#     plt.hist(post_tb[CH_A].values[idx_m_post], bins=40, density=True, alpha=0.35,
#              label="tBilby posterior CH log10_A")

# # Matt samples
# if A_CH_p is not None:
#     plt.hist(A_CH_p, bins=40, density=True, alpha=0.35,
#              label="Enterprise CH log10_A")

# plt.xlabel("CH log10_A")
# plt.ylabel("Density")
# plt.legend()
# plt.title("Dispersion measure amplitude")

# # -------- PANEL 2: gamma --------
# plt.subplot(1,2,2)

# # tBilby posterior samples
# if CH_G in post_tb.columns:
#     plt.hist(post_tb[CH_G].values[idx_m_post], bins=40, density=True, alpha=0.35,
#              label="tBilby posterior CH gamma")

# # Matt samples
# if g_CH_p is not None:
#     plt.hist(g_CH_p, bins=40, density=True, alpha=0.35,
#              label="Enterprise CH gamma")

# plt.xlabel("CH gamma")
# plt.ylabel("Density")
# plt.legend()
# plt.title("Dispersion measure spectral index")

# plt.tight_layout()
# plt.savefig(os.path.join(DIR_SAVE,f"{PULSAR}_CH_enterprise_full_model.pdf"))
# print(f"Saved CH distribution plot → {PULSAR}_CH_enterprise_full_model.pdf")

# f0 = fref
# j_tb  = np.nanmedian(A_CH)   ; g_tb  = np.nanmedian(g_CH)
# j_mt  = np.nanmedian(A_CH_p) ; g_mt  = np.nanmedian(g_CH_p)

# P_tb = psd_pow(np.array([f0]), j_tb,  g_tb)[0]
# P_mt = psd_pow(np.array([f0]), j_mt,  g_mt)[0]
# print("ratio P_tb / P_mt =", P_tb / P_mt)
