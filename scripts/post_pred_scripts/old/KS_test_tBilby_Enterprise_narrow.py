...
# ------------ select window around peak of CH amplitude ------------
def get_peak_window(arr, width=0.5, nbins=60):
    """
    Find a 1D window [lo, hi] of given width (in log10 A) around the
    histogram peak of the array arr.
    """
    arr = np.asarray(arr)
...
# peak windows for each set
lo_tb, hi_tb, peak_tb = get_peak_window(A_CH, width=0.5)
lo_mt, hi_mt, peak_mt = get_peak_window(A_CH_p, width=0.5)

# take the overlap so both use the same amplitude range
lo_win = max(lo_tb, lo_mt)
hi_win = min(hi_tb, hi_mt)

mask_tb_peak = (A_CH >= lo_win) & (A_CH <= hi_win)
mask_mt_peak = (A_CH_p >= lo_win) & (A_CH_p <= hi_win)

print(f"\nPeak window in CH log10_A: [{lo_win:.2f}, {hi_win:.2f}]")
print(f"  tBilby peak at ~{peak_tb:.2f}, Matt peak at ~{peak_mt:.2f}")
print(f"  N in window: tBilby={mask_tb_peak.sum()}, Matt={mask_mt_peak.sum()}")
...
# --- KS tests restricted to the CH-peak window ---
print("\n--- KS test in CH-peak window only ---")

# amplitude in peak window
ks_A_peak = ks_2samp(A_CH[mask_tb_peak], A_CH_p[mask_mt_peak])
print(f"CH log10_A (peak window): KS stat = {ks_A_peak.statistic:.4f}, "
      f"p-value = {ks_A_peak.pvalue:.4f}")

# optional: gamma restricted to the same samples (coupling gamma to A window)
ks_g_peak = ks_2samp(g_CH[mask_tb_peak], g_CH_p[mask_mt_peak])
print(f"CH gamma   (same peak window indices): KS stat = {ks_g_peak.statistic:.4f}, "
      f"p-value = {ks_g_peak.pvalue:.4f}")
